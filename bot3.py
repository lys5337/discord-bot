...
import nextcord
from nextcord.ext import commands
>>>>>>> 34d75d30d2fc7a9fb7d094f3878561e140a48044
...
def play_next(ctx):
    ...
    if len(user) >= 1:
        if not vc.is_playing():
            ...
            del musictitle[0]
            del song_queue[0]
            vc.play(nextcord.FFmpegPCMAudio(URL,**FFMPEG_OPTIONS), after=lambda e: play_next(ctx))
...
async def 지금노래(ctx):
    if not vc.is_playing():
        ...
        await ctx.send(embed = nextcord.Embed(title = "지금노래", description = "현재 " + musicnow[0] + "을(를) 재생하고 있습니다.", color = 0x00ff00))
...
async def 일시정지(ctx):
    if vc.is_playing():
        vc.pause()
        await ctx.send(embed = nextcord.Embed(title = '일시정지', discription = musicnow[0] + '을(를) 일시정지 했습니다', color = 0x00ff00))
    else:
...
async def 다시재생(ctx):
    try:
        vc.resume()
        await ctx.send(embed = nextcord.Embed(title = '일시정지', discription = musicnow[0] + '을(를) 다시 재생했습니다', color = 0x00ff00))
    except:
        await ctx.send('지금 노래가 재생되지 않네요')
...
async def 노래끄기(ctx):
    if vc.is_playing():
        vc.stop()
        await ctx.send(embed = nextcord.Embed(title = '노래끄기', discription = musicnow[0] + '을(를) 종료했습니다', color = 0x00ff00))
    else:
        await ctx.send('지금 노래가 재생되지 않네요')
